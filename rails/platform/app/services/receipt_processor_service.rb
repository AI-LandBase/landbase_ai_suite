class ReceiptProcessorService
  ...
  SYSTEM_PROMPT = <<~PROMPT
    ...
    1枚の領収書・レシートでも、購入品目が**異なる勘定科目にまたがる**場合は、品目ごとに仕訳を分割してください。

    - 例: 「食材（仕入高）＋ 清掃用品（消耗品費）」が1枚のレシートに混在 → 2件の仕訳に分割
    - 同一勘定科目に収まる品目はまとめて1件の仕訳にする（無理に細分化しない）
    - 分割した各仕訳の借方金額の合計が、領収書の税込合計金額と一致すること
    - 分割が発生した（`transactions` が2件以上の）場合、各仕訳の `status` は担当者確認のため後段で自動的に `review_required` に設定される（AI 側で無理に設定する必要はない）
    - 勘定科目が判別できる単一品目のみ、または全品目が同一勘定科目の場合は、従来どおり1件の仕訳にする
    ...
    ただし、領収書・レシートにクレジットカード払いを示す記載がある場合（「クレジット」「VISA」「Mastercard」「JCB」「AMEX」「カード払い」等）は、貸方を **「未払金」** にしてください。

    ### クレジットカード末尾4桁の抽出

    領収書・レシートにクレジットカードの末尾4桁が印字されている場合（例: 「****1234」「末尾1234」「XXXX-XXXX-XXXX-1234」等）は、各仕訳の `card_last_four` にその4桁の数字を設定してください。印字がない場合は `null` にしてください。
    ...
    品目が異なる勘定科目にまたがる場合は、`transactions` 配列に複数の仕訳を格納してください
    （各仕訳の借方金額合計 = 領収書の税込合計金額）。
