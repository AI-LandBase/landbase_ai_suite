...
撤回（Withdrawn） — 2026-06-21

> **撤回の理由（2026-06-21）**: 設計思想の変更により、リポジトリを物理分割する方針を取り下げる。
> `landbase_ai_suite` を単一リポジトリとして維持し、各 AI 機能はリポジトリ分割ではなくモジュール境界で整理していく方針へ転換した。
> これに伴い、本 ADR の Phase 1〜4 を扱っていた issue #279 / #280 / #281 / #282 はクローズ済み。
> 本 ADR は記録として残すが、以降の設計判断の根拠としては使用しない。

当初ステータス: 採用（Accepted）

## 日付

2026-05-10（採用） / 2026-06-21（撤回）
